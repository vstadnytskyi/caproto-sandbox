...
class SyringePump(object):
    ...
    def iowrite(self, pv_dict = None):
        ...
        """
        if self.io_put_queue not None:
            self.io_put_queue.put(pv_dict)
        else:
...
class ServerIO(PVGroup):
    ...
    async def running(self, instance, async_lib):
        ...
        device.io_put_queue = self.io_put_queue
        device.io_get_queue = self.io_get_queue
